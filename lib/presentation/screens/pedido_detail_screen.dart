  ...
  Widget build(BuildContext context) {
    if (_pedidoId == null) {
      return Scaffold(
        ...
        body: const Center(
          child: Text('ID de pedido no proporcionado.'),
        ),
      );
    }
              ...
              actions: [
                IconButton(
                  ...
                  onPressed: () async {
                    ...
                    if (result == true) {
                      setState(() {
                        _pedidoDetailFuture = _pedidosApi.getPedidoById(_pedidoId!); // Refresh data
                      });
                      nav.pop(true);
                    }
                  },
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    ...
                    if (confirmDelete == true) {
                      final messenger = ScaffoldMessenger.of(context); // Capture ScaffoldMessenger before try-catch
                      try {
                        // await _pedidosApi.deletePedido(pedido.id); // Need to add deletePedido to PedidosApi
                        if (!mounted) return; // Check mounted after async gap
                        messenger.showSnackBar(
                          const SnackBar(content: Text('Pedido eliminado exitosamente! (Simulado)')),
                        );
                        nav.pop(true);
                      } catch (e) {
                        if (!mounted) return; // Check mounted after async gap
            ...
            body: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailRow('Fecha Entrega:', DateFormat('dd/MM/yyyy').format(DateTime.parse(pedido.fechaEntrega))),
                  _buildDetailRow('Estado:', pedido.estado),
                  _buildDetailRow('Notas:', pedido.notas ?? 'N/A'),
                  _buildDetailRow('Total:', '\$${pedido.total.toStringAsFixed(2)}'),
