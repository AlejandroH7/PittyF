...
class _DessertDetailScreenState extends State<DessertDetailScreen> {
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return Scaffold(
            backgroundColor: backgroundColor,
            appBar: AppBar(title: const Text('Error'), backgroundColor: primaryColor, foregroundColor: Colors.white),
            body: Center(child: Text('Error al cargar el postre: ${snapshot.error}')),
          );
        }
        ...
        if (!snapshot.hasData) {
          return Scaffold(
            ...
            body: const Center(child: Text('No se encontraron detalles del postre.')),
          );
        }
            ...
            slivers: [
              SliverAppBar(
                ...
                flexibleSpace: FlexibleSpaceBar(
                  title: Text(dessert.nombre, style: const TextStyle(fontFamily: 'Georgia', fontWeight: FontWeight.bold)),
                  background: Stack(
                    fit: StackFit.expand,
                    children: [
                      ...
                      const Center(
                        child: Icon(Icons.cake_outlined, color: Colors.white, size: 100),
                      ),
                    ],
                  ),
                ),
                actions: [
                  IconButton(icon: const Icon(Icons.edit), onPressed: () => _editDessert(dessert)),
                  IconButton(icon: const Icon(Icons.delete), onPressed: () => _deleteDessert(dessert)),
                ],
              ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      _InfoCard(
                        ...
                        children: [
                          _DetailRow(icon: Icons.attach_money, label: 'Precio', value: NumberFormat.simpleCurrency(locale: 'es_MX').format(dessert.precio)),
                          _DetailRow(icon: Icons.pie_chart_outline, label: 'Porciones', value: '${dessert.porciones} porciones'),
                          _DetailRow(icon: dessert.activo ? Icons.toggle_on : Icons.toggle_off, label: 'Estado', value: dessert.activo ? 'Activo' : 'Inactivo', valueColor: dessert.activo ? Colors.green : Colors.red),
                        ],
                      ),
                      ...
                      _InfoCard(
                        ...
                        children: [
                          _DetailRow(icon: Icons.fingerprint, label: 'ID de Postre', value: dessert.id.toString()),
                          _DetailRow(icon: Icons.calendar_today_outlined, label: 'Fecha de Creación', value: _formatDate(dessert.createdAt)),
                          _DetailRow(icon: Icons.calendar_today, label: 'Última Actualización', value: _formatDate(dessert.updatedAt)),
                        ],
                      ),
                    ],
                  ),
  ...
  void _editDessert(DessertModel dessert) async {
    final result = await Navigator.of(context).pushNamed(DessertEditScreen.routeName, arguments: dessert);
    if (result == true && mounted) {
      _refreshDessertDetails();
      ScaffoldMessenger.of(context)..removeCurrentSnackBar()..showSnackBar(const SnackBar(content: Text('Postre actualizado'), backgroundColor: Colors.green));
    }
  }
  ...
  void _deleteDessert(DessertModel dessert) async {
    final bool? confirmDelete = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Estás seguro de que quieres eliminar ${dessert.nombre}?'),
        actions: [
    ...
    if (confirmDelete == true && mounted) {
      try {
        ...
        String errorMessage = e.response?.data?['message'] ?? 'Error al eliminar el postre.';
